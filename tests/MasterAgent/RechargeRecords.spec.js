...
test('Verify Recharge Records for all status filters', async ({ page }) => {
  // ==== STEP 1: Agent Login ====
    await page.goto('https://skynbliss.co/login', { timeout: 120000 });
  await page.locator('input[name="emailPhone"]').fill('Agent123@gmail.com');
  await page.getByRole('button', { name: 'Next' }).click();
  ...
  for (const status of statuses) {
    ...
    // status column index (0-based). Adjust if your table layout differs.
    // In your screenshots, Status is the 5th column -> index = 4
    ...
    // Wait until either table has 0 rows OR at least one row's status cell includes the selected status.
    try {
      await page.waitForFunction(
        ([s, idx]) => {
          ...
          if (rows.length === 0) return true; // no records -> acceptable
          return rows.some(r => {
        ...
        { timeout: 15000 } // tune as needed
      );
    } catch (err) {
      // timed out waiting for table to update
      console.log(`⚠️ Timed out waiting for results for status: ${status}`);
      // fall through to inspect actual rows for final decision
    }
    ...
    // If not "All", validate each row's status cell matches selected status
    if (status !== 'All') {
